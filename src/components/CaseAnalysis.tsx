          ...
          <p key={i} className="text-sm leading-relaxed">
            {parts.map((part, j) =>
              part.startsWith("**") && part.endsWith("**")
                ? <strong key={j} className="font-semibold text-white/95">{part.slice(2, -2)}</strong>
                : <span key={j} className="text-white/75">{part}</span>
            )}
          </p>
        );
      })}
    ...
    >
      ...
      {analysis["رؤى_استراتيجية"] && (
        ...
        >
          ...
          <RichText text={analysis["رؤى_استراتيجية"]} />
        </motion.div>
      )}
