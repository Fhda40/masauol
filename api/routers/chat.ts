...
${hasKbContext ? "7. استشهد بالمواد القانونية: 'حسب المادة [X] من [النظام]'" : ""}`;
      ...
      const analysisRecord: Record<string, unknown> = {
        ...
        "فهم الحالة": finalOutput.sections?.["فهم_الحالة"] ?? deepAnalysis.factsExtracted,
        "التكييف القانوني": finalOutput.sections?.["التكييف_القانوني"] ?? `الأنظمة: ${deepAnalysis.applicableLaws.join(", ")} — ${deepAnalysis.legalPosition}`,
      ...
      if (allMessages.length <= 3) {
        ...
        await getDb()
          ...
          .set({
            title: title + "...",
            caseType: deepAnalysis.caseType as any,
            updatedAt: new Date(),
          })
